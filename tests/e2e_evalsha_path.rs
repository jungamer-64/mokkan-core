...
async fn script_loads_and_evalsha_path_behavior() {
    ...
    let store = RedisSessionRevocationStore::from_url_with_options(&url, 60 * 60 * 24 * 7, false)
        .expect("create store");
    ...
    store
        .set_session_refresh_nonce(&session_id, "n1")
        .await
        .expect("set nonce");
    let ok = store
        .compare_and_swap_session_refresh_nonce(&session_id, "n1", "n2")
        .await
        .expect("cas");
    ...
    assert_eq!(
        store.script_loads(),
        1,
        "script should have been loaded once"
    );

    // Second CAS (rotate n2 -> n3) should use EVALSHA (no additional SCRIPT LOAD)
    let ok2 = store
        .compare_and_swap_session_refresh_nonce(&session_id, "n2", "n3")
        .await
        .expect("cas2");
    ...
    assert_eq!(
        store.script_loads(),
        1,
        "script should not have been loaded again"
    );
    ...
    let ok3 = store
        .compare_and_swap_session_refresh_nonce(&session_id, "n3", "n4")
        .await
        .expect("cas3");
    ...
    assert_eq!(
        store.script_loads(),
        2,
        "script should have been loaded a second time after NOSCRIPT"
    );
}
