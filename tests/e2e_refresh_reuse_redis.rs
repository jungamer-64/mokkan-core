...
impl mokkan_core::application::ports::security::TokenManager for FakeTokenManager {
    ...
    > {
        Err(mokkan_core::application::error::ApplicationError::unauthorized(
    ...
    async fn public_jwk(
        &self,
    ) -> mokkan_core::application::ApplicationResult<serde_json::Value> {
...
async fn refresh_token_single_use_with_redis_store() {
    ...
    impl mokkan_core::domain::user::repository::UserRepository for InMemoryUserRepo {
        async fn count(
            &self,
        ) -> mokkan_core::domain::errors::DomainResult<u64> {
        ...
        async fn insert(
            ...
            mokkan_core::domain::user::entity::User,
        > {
        ...
        > {
            ...
            let user = map
                .get_mut(&id)
                .ok_or_else(|| {
                    mokkan_core::domain::errors::DomainError::NotFound(
                        "user not found".into(),
                    )
                })?;
    ...
    let password_hasher: Arc<
        dyn mokkan_core::application::ports::security::PasswordHasher,
    > = Arc::new(support::mocks::DummyPasswordHasher);
    let token_manager: Arc<
        dyn mokkan_core::application::ports::security::TokenManager,
    ...
    let h1 = tokio::spawn(async move {
        svc1.refresh_token(RefreshTokenCommand { token: tkn }).await
    });

    let svc2 = svc.clone();
    let tkn2 = refresh_token2.clone();
    let h2 = tokio::spawn(async move {
        svc2.refresh_token(RefreshTokenCommand { token: tkn2 }).await
    });
