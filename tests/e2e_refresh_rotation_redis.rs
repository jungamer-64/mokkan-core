...
async fn refresh_token_single_use_with_redis_store() {
    ...
    let login = svc.login(LoginUserCommand { username: "redis_user".into(), password: "pwd".into() }).await.expect("login");
    ...
    let login2 = svc.login(LoginUserCommand { username: "redis_user".into(), password: "pwd".into() }).await.expect("login2");
